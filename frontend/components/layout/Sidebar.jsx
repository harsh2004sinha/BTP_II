...
const navItems = [
  ...
  { href: "/plans/new", label: "New Plan",  icon: PlusCircle,      exact: true  },
  { href: "/plans",     label: "My Plans",  icon: FolderOpen,      exact: false },
...
function isNavActive(pathname, href, exact) {
  ...
  /* must start with href + "/" */
  if (!pathname.startsWith(href + "/")) return false;

  /* exclude /plans/new — it has its own exact nav item */
  const excluded = ["/plans/new"];
  if (excluded.some((ex) => pathname === ex || pathname.startsWith(ex + "/"))) {
    return false;
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-800">
        <div
          className="shrink-0 w-8 h-8 bg-emerald-500 rounded-lg
